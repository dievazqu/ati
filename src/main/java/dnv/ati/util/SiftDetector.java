...
//		String lib = "/Users/michelagopian/Desktop/opencv/build/lib/libopencv_java2413.dylib";
	...
	private MatOfKeyPoint calculateMatOfKeyPoints(Mat image, int sideK) {
		...
		for (int i = 0; i < sideK; i++) {
			for (int j = 0; j < sideK; j++) {
				keypoints[i * sideK + j] = new KeyPoint(stepWidth * (i + 1.5f),
						stepHeight * (j + 1.5f), 1);
			}
		}
	...
	public String detectFace(String img1) {
		...
		File folder = new File(trainingSetFolder);
		File[] listOfFiles = folder.listFiles();
		List<String> imageSet = new ArrayList<String>();
		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile()) {
				imageSet.add(listOfFiles[i].getAbsolutePath());
			}
		}

		Mat firstImage = Highgui.imread(img1, Highgui.CV_LOAD_IMAGE_COLOR);
		MatOfKeyPoint firstImageMatOfKeyPoints = calculateMatOfKeyPoints(
				firstImage, sideK);
		...
		descriptorExtractor.compute(firstImage, firstImageMatOfKeyPoints,
				firstImageDescriptors);
		int idx = 0;
		double minDistance = compareFaceImages(firstImageDescriptors,
				imageSet.get(0));
		for (int i = 1; i < imageSet.size(); i++) {
			double auxDistance = compareFaceImages(firstImageDescriptors,
					imageSet.get(i));
	...
	private double compareFaceImages(MatOfKeyPoint firstImageDescriptors,
		...
		DescriptorExtractor descriptorExtractor = DescriptorExtractor
				.create(DescriptorExtractor.SIFT);
		MatOfKeyPoint secondImageMatOfKeyPoints = calculateMatOfKeyPoints(
				secondImage, sideK);
		MatOfKeyPoint secondImageDescriptors = new MatOfKeyPoint();
		descriptorExtractor.compute(secondImage, secondImageMatOfKeyPoints,
				secondImageDescriptors);
		...
		descriptorMatcher.knnMatch(firstImageDescriptors,
				secondImageDescriptors, matches, sideK * sideK);
